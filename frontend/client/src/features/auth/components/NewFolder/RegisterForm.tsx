...
export interface RegisterFormProps {
  /** Callback invoked after a successful registration across any role */
  onSuccess: () => void;
}
...
export function RegisterForm({ onSuccess }: RegisterFormProps) {
  ...
  const handleNextStep = (e: React.FormEvent) => {
    ...
    // Advance to role-specific details
    setStep(2);
  };

  // Bundle primary account info to pass to the final submission step
  const primaryAccountData = { fullName, email, role, password };

  return (
    <div className="max-w-xl mx-auto bg-white p-6 rounded-xl shadow-sm border border-gray-100 font-sans">
      {/* Header section */}
      <div className="flex flex-col items-center mb-6">
        ...
        <p className="text-gray-400 text-sm mt-1">
          {step === 1 ? 'Step 1: Create your account credentials' : `Step 2: Complete your ${role.replace('_', ' ')} profile`}
        </p>
      </div>

      {error && (
        <p className="text-sm text-red-500 bg-red-50 p-2 rounded border border-red-200 mb-4">
          {error}
        </p>
      )}
      ...
      {step === 1 ? (
        <form onSubmit={handleNextStep} className="space-y-4">
          ...
          <button
            type="submit"
            className="w-full mt-4 py-3 bg-blue-500 hover:bg-blue-600 text-white font-semibold rounded-xl transition-colors shadow-sm flex items-center justify-center gap-2"
          >
            NEXT
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div>
          ...
          <button
            onClick={() => setStep(1)}
            className="text-sm font-semibold text-blue-500 hover:text-blue-600 flex items-center gap-1 mb-4"
          >
          ...
          {/* Conditionally Render Step 2 Content */}
          {role === 'tourist' && (
            <TouristForm accountData={primaryAccountData} onSuccess={onSuccess} />
          )}
          {role === 'guide' && (
            <GuideForm accountData={primaryAccountData} onSuccess={onSuccess} />
          )}
          {role === 'hotel_owner' && (
            <HotelForm accountData={primaryAccountData} onSuccess={onSuccess} />
          )}
        </div>
      )}
    </div>
