...
export default function GuideForm({ accountData, onSuccess }: GuideFormProps) {
  ...
  const [certificationPhoto, setCertificationPhoto] = useState<File | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('Submitting Tour Guide Application:', {
        ...
        bankDetails,
        certificationPhotoName: certificationPhoto?.name,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label className="block text-sm font-semibold text-gray-700 mb-1">Spoken Languages (Comma Separated)</label>
        <input
          type="text"
          required
      ...
      <div>
        <label className="block text-sm font-semibold text-gray-700 mb-1">Bank Details (Bank, Branch, Acc No)</label>
        <textarea
          required
          rows={2}
      ...
      <div className="border border-dashed border-gray-200 rounded-lg p-4 bg-gray-50 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1">Guide License / Certification</label>
          <input
            type="file"
            required
            accept="image/*"
            onChange={(e) => setCertificationPhoto(e.target.files?.[0] || null)}
        ...
        <div>
          <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1">NIC or Passport Front</label>
          <input
            type="file"
            required
            accept="image/*"
